...
// pra não sair o undefined da função, basta colocar o setTimeout dentro dela
...
const sendMarsTemperature = (callback, error) => {
  setTimeout(() => {
    if (Math.random() <= 0.6) { // para considerar os 60%
      callback(getMarsTemperature())
    } else {
      error('Error getting temperature: Robot is busy')
    }
  }, messageDelay())
}
