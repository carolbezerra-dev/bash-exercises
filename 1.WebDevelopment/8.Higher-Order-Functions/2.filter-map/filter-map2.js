...
function authorWith3DotsOnName() {
  const justDot = books.filter((element) => {
  return element.author.name.slice(0, 8) == `${element.author.name[0]}. ${element.author.name[3]}. ${element.author.name[6]}.`});

  return justDot.map((param) => param.name)[0];
